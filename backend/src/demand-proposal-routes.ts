...
export function registerDemandProposalRoutes(app: Application) {
  ...
  app.get("/api/admin/demand/proposals", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const { rows } = await pool.query(`
        ...
        ORDER BY CASE WHEN p.status='draft' THEN 1 WHEN p.status='partial' THEN 2 ELSE 3 END,
                 p.created_at DESC
  ...
  app.post("/api/admin/demand/proposals/:id/status", requireAdmin, async (req: Request, res: Response) => {
    ...
    try {
      const { rows } = await pool.query(`
        UPDATE demand_proposals SET status=$2,updated_at=NOW() WHERE id=$1 RETURNING *`,
  ...
  // Link compartilhável somente leitura. Não expõe contatos, custo-fonte nem margem.
