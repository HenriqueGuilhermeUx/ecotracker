...
export function registerAssistedSourcingOpsRoutes(app: Application) {
  ...
  app.post("/api/admin/market/assisted-sourcing/:id/confirm-source", requireAdmin, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (String(row.payment_status || "not_started") === "paid") return res.status(409).json({ error: "Pagamento já foi confirmado; preço da fonte não pode mais ser alterado" });
      ...
      res.json({
        ...
        message: "Fonte, estoque e custo foram confirmados. O pagamento pode ser liberado; aquisição e aposentadoria continuarão assistidas.",
      });
    } catch (error) { fail(res, error); }
  });
