...
async function rfqView(id:number) {
  const { rows } = await pool.query(`
    SELECT r.*,a.company_name,a.tax_id,a.sector,a.contact_name,a.contact_email,
      ...
      ) ORDER BY c.sourcing_score DESC,c.candidate_tonnes DESC,c.id) FROM market_maker_rfq_candidates c WHERE c.rfq_id=r.id),'[]'::jsonb) AS candidates
...
export async function refreshDemandSupplyRfqCandidates(rfqId:number) {
  return withTransaction(async (client) => {
    ...
    if (["resolved","cancelled"].includes(String(rfq.status))) return rfqView(rfqId);
    ...
    for (const row of inventory.rows) {
      ...
      const rationale = {
        basis:"supplier_mandate",
        commercialInventoryConfirmed:true,
        claimReady:false,
        warning:"Mandated supply is a sourcing candidate; it only resolves demand after entering the monitored claim-ready catalog and passing eligibility.",
      };
      const snapshot = {
        supplierName:row.supplier_name,batchReference:row.batch_reference,availableTonnes:tonnes,
        floorPriceUsdTonne:row.floor_price_usd_tonne,registryEvidenceUrl:row.registry_evidence_url,
      };
      await client.query(`
        ...
        tonnes,score,JSON.stringify(rationale),JSON.stringify(snapshot),
      ]);
    }
    ...
    for (const row of leads.rows) {
      ...
      const rationale = sellerConfirmed ? {
        basis:"seller_confirmed_free_inventory",
        commercialInventoryConfirmed:true,
        mandateRequired:true,
        claimReady:false,
      } : {
        basis:"registry_estimate_only",
        commercialInventoryConfirmed:false,
        mandateRequired:true,
        claimReady:false,
        warning:"Registry balance estimate is not commercially free inventory until the supplier confirms it.",
      };
      const snapshot = {
        supplierName:row.supplier_name,supplierContactName:row.supplier_contact_name,supplierEmail:row.supplier_email,
        supplierPhone:row.supplier_phone,evidenceUrl:row.evidence_url,sourceUrl:row.source_url,
        availabilityConfidence:row.availability_confidence,contactStatus:row.contact_status,
      };
      await client.query(`
        INSERT INTO market_maker_rfq_candidates(
        ...
        tonnes,sellerConfirmed ? "seller_confirmed" : "registry_estimate",score,JSON.stringify(rationale),JSON.stringify(snapshot),
      ]);
    }

    const liveCandidates = await client.query(`
      SELECT COALESCE(SUM(candidate_tonnes),0) AS candidate_tonnes,
             COUNT(*) FILTER (WHERE status<>'stale')::int AS candidate_count
      FROM market_maker_rfq_candidates WHERE rfq_id=$1 AND status<>'stale'`,[rfqId]);
    const candidateTonnes = num(liveCandidates.rows[0]?.candidate_tonnes);
    const nextStatus = candidateTonnes>0 ? "partially_sourced" : "open";
    await client.query(`
...
export async function upsertDemandSupplyRfq(input:{
  ...
  const result = await withTransaction(async (client) => {
    ...
    if (input.gapTonnes<=0.001) {
      const resolved = (await client.query(`
        ...
        WHERE opportunity_id=$1 RETURNING *`,[input.opportunityId,input.coveredTonnes])).rows[0];
      return resolved || null;
    }
    const preferredCountry = opportunity.preferred_country || opportunity.account_country || null;
    const requirements = {
    ...
    return (await client.query(`
      ...
      input.targetTonnes,input.coveredTonnes,input.gapTonnes,preferredCountry,opportunity.max_price_usd_tonne,
      Math.max(0,Math.min(100,Math.round(num(opportunity.priority_score,50)))),JSON.stringify(requirements),input.source || "demand_autopilot",
    ])).rows[0];
  });
...
export async function getDemandSupplyRfq(id:number) {
  return rfqView(id);
}
...
export async function marketMakerSummary() {
  const { rows } = await pool.query(`
    SELECT
      COUNT(*) FILTER (WHERE status IN ('open','partially_sourced'))::int AS open_rfqs,
