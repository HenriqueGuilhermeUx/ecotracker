...
async function run(){
  ...
  const retirementProvider=async(input)=>{orders++;const technical=input.quoteUuid.includes("technical");const fill=(technical?"b":"a").repeat(64);return{status:"completed",reference:`retirement-${technical?"technical-":""}${tag}`,txHash:`0x${fill}`,viewRetirementUrl:`https://sandbox.example/retirement/${technical?"technical-":""}${tag}`,certificateUrl:`https://sandbox.example/certificate/${technical?"technical-":""}${tag}`,provenanceUrl:`https://sandbox.example/provenance/${technical?"technical-":""}${tag}`,retirementId:`137-0x${fill}-0`,raw:{status:"COMPLETED",sandbox:true,technical}};};
